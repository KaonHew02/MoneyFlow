...
/**
 * Close a payment without paying it.
 *
 * A month is forgiven, written off, or simply never going to be handed over —
 * and until now the only way to get it off the schedule was to tick it, which
 * says money moved and puts an expense in the book for money nobody spent.
 * Waiving closes the month in the muted ink instead: it comes off what is
 * left, it keeps its own figure apart from what was paid, and it writes
 * nothing into Expenses.
 */
function commitWaivePayment(n) {
    const plan = commitDraft();
...
/**
 * --------------------------------------------------------------------
 * One module falling over is not seven
 * --------------------------------------------------------------------
 * `startApp` used to be one straight run of calls. A throw anywhere in it —
 * a bill in a shape the code did not expect, an instalment pointing at an
 ...
 * when something has gone wrong is usually Export, and it is the last to load.
 */
...
function startApp() {
    ...
    // others with it. The order is unchanged and still matters.
